...
type ModalState =
  | { phase: 'idle' }
  | { phase: 'confirm'; classId: string; className: string }
  | { phase: 'deleting'; classId: string; className: string }
  | { phase: 'error' };
...
export function ClassesList({ classes: initialClasses }: { classes: ClassItem[] }) {
  ...
  function requestDelete(item: ClassItem) {
    setModal({ phase: 'confirm', classId: item.id, className: item.name });
  }

  function confirmDelete() {
    if (modal.phase !== 'confirm') return;
    const { classId, className } = modal;
    setModal({ phase: 'deleting', classId, className });
    startTransition(async () => {
      try {
        const res = await fetch(`/api/institution/classes/${classId}`, { method: 'DELETE' });
  ...
  const className = modal.phase === 'confirm' || modal.phase === 'deleting' ? modal.className : '';
  ...
  return (
    <>
      <FeedbackModal
        ...
        title={isError ? 'No se pudo eliminar la clase' : '¿Eliminar clase?'}
        titleColor="text-[#D86262]"
        description={
          isError
            ? <>Ocurrió un problema al procesar la solicitud.<br />Por favor, inténtalo nuevamente.</>
            : <>Estás a punto de eliminar <span className="font-medium text-brand-brown">{className}</span>.<br />Esta acción es irreversible.</>
        }
        actions={
          isError ? (
      ...
      <div className="rounded-2xl px-10 card-shadow overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5">
          <span className="text-sm font-semibold text-brand-brown">Clases registradas</span>
                  ...
                  <tr key={item.id}>
                    <td className="pl-6 pr-4 py-4 border-t border-brand-border max-w-0">
                      <div>
                        ...
                        <span className="block truncate text-xs text-brand-placeholder mt-0.5 font-mono">{item.license_code}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 border-t border-brand-border">
                      <span className="text-sm text-brand-brown">{item.period}</span>
                    </td>
                    <td className="px-4 py-4 border-t border-brand-border">
        ...
        <div className="flex items-center justify-between px-6 py-4 pt-10 border-t border-brand-border">
          <span className="text-sm text-brand-label">
            Mostrando {Math.min(paginated.length, PAGE_SIZE)} de {filtered.length} clases
          </span>
