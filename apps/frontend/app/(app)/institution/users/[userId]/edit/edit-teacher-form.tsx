...
import type { TeacherItem } from '@/lib/api/teacher';
...
export function EditTeacherForm({ teacher, courses, subjects, classes }: Props) {
  ...
  const initialSubjectIds = teacher.subjects.map((s) => s.id);
  const initialClassIds = teacher.classes.map((c) => c.id);
  const initialCourseIds = Array.from(
    new Set([
      ...teacher.subjects.map((s) => s.course.id),
      ...teacher.classes.map((c) => c.course.id),
    ]),
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (selectedCourseIds.length === 0) {
      setErrorMsg('Selecciona al menos un curso.');
      setIsSuccess(false);
      setShowModal(true);
      return;
    }
    ...
    if (selectedSubjectIds.length === 0) {
      setErrorMsg('Selecciona al menos una asignatura.');
      setIsSuccess(false);
      setShowModal(true);
      return;
    }
    ...
    if (selectedClassIds.length === 0) {
      setErrorMsg('Selecciona al menos una clase.');
      setIsSuccess(false);
      setShowModal(true);
      return;
    }
    ...
    try {
      const res = await fetch(`/api/institution/teachers/${teacher.userId}`, {
        method: 'PATCH',
  ...
  return (
    <>
      <FeedbackModal
        ...
        title={isSuccess ? '¡Profesor actualizado!' : 'No pudimos guardar los cambios'}
      ...
      <div className="pt-10 px-10 md:px-30 pb-16">
        ...
        <div className="mb-10">
          <h1 className="text-4xl text-brand">Editar usuario / Profesor</h1>
          <p className="text-sm text-brand-label mt-1">
            Crea y administra usuarios dentro de la plataforma de la institución.
          </p>
        </div>
