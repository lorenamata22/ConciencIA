...
export function CreateTeacherForm({ courses, subjects, classes }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/institution/teachers', {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(json.message ?? 'No se pudo registrar el profesor. Inténtalo de nuevo.');
        return;
      }
      ...
      setError('No se pudo registrar el profesor. Inténtalo de nuevo.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleSendEmail() {
    ...
    try {
      const res = await fetch(`/api/institution/teachers/${createdUserId}/send-access-email`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="pt-10 px-10 md:px-30 pb-16">
        ...
        <div className="max-w-xl">
          ...
          <h1 className="text-3xl text-brand mb-4">
            Registro del<br />profesor completado
          </h1>
          ...
          <p className="text-sm text-brand-brown mb-6">
            Hemos generado un código único para que el docente<br />
            pueda completar su proceso de registro y acceder a la plataforma.
          </p>
  ...
  return (
    <div className="pt-10 px-10 md:px-30 pb-16">
      ...
      <div className="mb-10">
        <h1 className="text-4xl text-brand">Nuevo usuario / Profesor</h1>
