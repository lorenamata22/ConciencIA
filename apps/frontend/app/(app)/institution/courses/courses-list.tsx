...
type ModalState =
  | { phase: 'idle' }
  | { phase: 'confirm'; courseId: string; courseName: string }
  | { phase: 'deleting'; courseId: string; courseName: string }
  | { phase: 'error' };
...
export function CoursesList({ courses: initialCourses }: { courses: CourseOption[] }) {
  ...
  function requestDelete(course: CourseOption) {
    setModal({ phase: 'confirm', courseId: course.id, courseName: course.name });
  }

  function confirmDelete() {
    if (modal.phase !== 'confirm') return;
    const { courseId, courseName } = modal;
    setModal({ phase: 'deleting', courseId, courseName });
    startTransition(async () => {
      try {
        const res = await fetch(`/api/institution/courses/${courseId}`, { method: 'DELETE' });
  ...
  const courseName = modal.phase === 'confirm' || modal.phase === 'deleting' ? modal.courseName : '';
  ...
  return (
    <>
      <FeedbackModal
        ...
        title={isError ? 'No se pudo eliminar el curso' : '¿Eliminar curso?'}
        titleColor="text-[#D86262]"
        description={
          isError
            ? <>Ocurrió un problema al procesar la solicitud.<br />Por favor, inténtalo nuevamente.</>
            : <>Estás a punto de eliminar <span className="font-medium text-brand-brown">{courseName}</span>.<br />Se eliminarán también todas sus asignaturas. Esta acción es irreversible.</>
        }
        actions={
          isError ? (
      ...
      <div className="rounded-2xl px-10 card-shadow overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5">
          <span className="text-sm font-semibold text-brand-brown">Cursos registrados</span>
              ...
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-sm text-brand-placeholder border-t border-brand-border">
                    {search ? 'Ningún curso coincide con la búsqueda.' : 'No hay cursos registrados.'}
                  </td>
                </tr>
              ) : (
                paginated.map((course) => (
                  <tr key={course.id}>
                    <td className="pl-6 pr-4 py-4 border-t border-brand-border max-w-0">
                      <span className="block truncate text-sm font-medium text-brand-brown">{course.name}</span>
                    </td>
        ...
        <div className="flex items-center justify-between px-6 py-4 pt-10 border-t border-brand-border">
          <span className="text-sm text-brand-label">
            Mostrando {Math.min(paginated.length, PAGE_SIZE)} de {filtered.length} cursos
          </span>
