...
export class EmailService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.fromAddress = this.config.get<string>('EMAIL_FROM', 'ConciencIA <no-reply@conciencia.education>');
  ...
  async sendAccessInvite(
    ...
    const registrationUrl = `${this.frontendUrl}/complete-registration?code=${accessCode}`;
    const roleLabel = userType === 'teacher' ? 'profesor' : 'estudiante';

    const { error } = await this.resend.emails.send({
      from: this.fromAddress,
      to: toEmail,
      subject: 'Acceso a la plataforma — ConciencIA',
      html: buildAccessInviteEmail(name, accessCode, registrationUrl, roleLabel),
      text: buildAccessInviteEmailText(name, accessCode, registrationUrl, roleLabel),
    });

    if (error) {
      this.logger.error(`Falha ao enviar email de acesso para ${toEmail}: ${error.message}`);
      throw new Error('Falha ao enviar email de acesso.');
    }

    this.logger.log(`Email de acesso enviado para ${toEmail}`);
  }
}

function buildAccessInviteEmailText(
  name: string,
  accessCode: string,
  registrationUrl: string,
  roleLabel: string,
): string {
  return `Bienvenido/a a ConciencIA

Hola ${name},

Has sido registrado/a como ${roleLabel} en la plataforma ConciencIA.
Para finalizar tu registro y acceder, utiliza el siguiente código de acceso:
...
function buildAccessInviteEmail(
  name: string,
  accessCode: string,
  registrationUrl: string,
  roleLabel: string,
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              <p style="margin:0 0 8px;font-size:20px;font-weight:700;color:#3D2B1F;">Bienvenido/a a ConciencIA</p>
              <p style="margin:0 0 24px;font-size:14px;color:#7A6A5F;line-height:1.6;">
                Hola ${name}, has sido registrado/a como <strong>${roleLabel}</strong> en la plataforma.
                Utiliza el código de acceso a continuación para finalizar tu registro.
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="padding:0 0 24px;">
                    <div style="display:inline-block;background-color:#F5F3EE;border-radius:12px;padding:14px 28px;">
                      <span style="font-size:20px;font-weight:700;letter-spacing:4px;color:#3D2B1F;">${accessCode}</span>
                    </div>
                  </td>
                </tr>
              </table>
              ...
              <p style="margin:0 0 24px;font-size:12px;word-break:break-all;">
                <a href="${registrationUrl}" style="color:#85C9C3;">${registrationUrl}</a>
              </p>
...
Este enlace expira en 1 hora. Si no solicitaste restablecer tu contraseña, puedes ignorar este mensaje.
