...
export class InstitutionService {
  ...
  async update(id: string, dto: UpdateInstitutionDto) {
    ...
    return this.prisma.institution.update({
      ...
      data: {
        ...
        ...(dto.aiTokenLimit !== undefined && {
          ai_token_limit: dto.aiTokenLimit,
        }),
        ...(dto.subjectLimit !== undefined && {
          subject_limit: dto.subjectLimit,
        }),
      },
    });
  ...
  async remove(id: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.studentMetrics.deleteMany({
        where: { student_id: { in: studentIds } },
      });
  ...
  async getDetailStats(id: string) {
    ...
    const usedTokens =
      (tokenTotal._sum.prompt_tokens ?? 0) +
      (tokenTotal._sum.response_tokens ?? 0);
    ...
    const inputCost =
      monthTotal > 0 ? monthCost * (monthPrompt / monthTotal) : 0;
    const outputCost =
      monthTotal > 0 ? monthCost * (monthResponse / monthTotal) : 0;

    return {
      tokenUsage: {
        usedTokens,
        remainingTokens: tokenLimit
          ? Math.max(0, tokenLimit - usedTokens)
          : null,
        tokenLimit,
        usagePercent,
