...
export class StudentService {
  ...
  async submitCognitiveTest(studentId: string, profile: object) {
    ...
    if (student.test_count >= 3) {
      throw new ForbiddenException('Limite de 3 tentativas de teste cognitivo atingido.');
    }
  ...
  async createByInstitution(dto: CreateStudentInstitutionDto, institutionId: string) {
    ...
    if (!classRecord) throw new NotFoundException('Turma não encontrada ou não pertence à instituição');
    ...
    const student = await this.prisma.$transaction(async (tx) => {
      ...
      return { studentId: created.id, userId: user.id, name: user.name, email: user.email };
    });
  ...
  async findAllByInstitution(institutionId: string) {
    const students = await this.prisma.student.findMany({
      ...
      select: {
        id: true,
        user: { select: { id: true, name: true, email: true, phone: true, is_minor: true } },
        studentClasses: {
          select: {
            class: {
  ...
  async findOneByUserId(userId: string, institutionId: string) {
    const student = await this.prisma.student.findUnique({
      ...
      select: {
        id: true,
        user: {
          select: { id: true, name: true, email: true, phone: true, is_minor: true, institution_id: true },
        },
        studentClasses: {
          select: {
            class: {
    ...
    if (!student) throw new NotFoundException('Estudante não encontrado');
    if (student.user.institution_id !== institutionId) throw new ForbiddenException('Acesso negado');
  ...
  async updateByUserId(userId: string, dto: UpdateStudentInstitutionDto, institutionId: string) {
    const student = await this.prisma.student.findUnique({
      where: { user_id: userId },
